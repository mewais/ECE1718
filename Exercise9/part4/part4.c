...
#define VIDEO_SIZE 256
#define KEY_SIZE 256
#define ADC_BASE 0xFF204000
#define ADC_SPAN 0x100

#define ADC_VOLTAGE_SCALE 4096
#define ADC_SWEEP_SCALE 312500	// ns

int screen_x, screen_y;
int y_scale;
static int video_FD, key_FD, mem_FD; // file descriptor
...
{
	.it_interval = {.tv_sec=0,.tv_nsec=ADC_SWEEP_SCALE},
	.it_value = {.tv_sec=0,.tv_nsec=ADC_SWEEP_SCALE}
};
...
void handle_sigalrm(int dummy)
{
	if (oscilloscope_index < screen_x)
	{
		...
		{
			// Stop the timer
			int ret = timer_settime (interval_timer_id, 0, &interval_timer_stop, NULL);
			if (ret != 0)
			{
				ret = timer_settime (interval_timer_id, 0, &interval_timer_stop, NULL);
			}
			// Actually plot
			int string_length;
			for (int x = 1; x < screen_x; x++)
			{
				sprintf(VIDEO_BUF, "line %i,%i %i,%i %04x%n", x-1, oscilloscope_array[x-1], x, oscilloscope_array[x], 0xF000, &string_length);
				write(video_FD, VIDEO_BUF, string_length);
			}
			sprintf(VIDEO_BUF, "sync%n", &string_length);
			write(video_FD, VIDEO_BUF, string_length);
		}
	}
}
...
int main(int argc, char *argv[])
{
	...
	{
		printf ("ERROR: mmap() failed for ADC...\n");
		close (mem_FD);
		return -1;
	}
	...
	while(read(video_FD, VIDEO_BUF, VIDEO_SIZE)!=0);
	screen_x = ((VIDEO_BUF[0] - '0') * 100) + ((VIDEO_BUF[1] - '0') * 10) + VIDEO_BUF[2] - '0';
	screen_y = ((VIDEO_BUF[4] - '0') * 100) + ((VIDEO_BUF[5] - '0') * 10) + VIDEO_BUF[6] - '0';
	oscilloscope_array = malloc(screen_x*sizeof(int));

	int string_length;
	sprintf(VIDEO_BUF, "clear%n", &string_length);
	write(video_FD, VIDEO_BUF, string_length);
	sprintf(VIDEO_BUF, "erase%n", &string_length);
	write(video_FD, VIDEO_BUF, string_length);
	sprintf(VIDEO_BUF, "sync%n", &string_length);
	write(video_FD, VIDEO_BUF, string_length);

	// Draw Axis
	sprintf(VIDEO_BUF, "line %i,%i %i,%i %04x%n", 0, 239, 319, 239, 0x0AC0, &string_length);
	write(video_FD, VIDEO_BUF, string_length);
	sprintf(VIDEO_BUF, "line %i,%i %i,%i %04x%n", 0, 0, 0, 239, 0x0AC0, &string_length);
	write(video_FD, VIDEO_BUF, string_length);
	sprintf(VIDEO_BUF, "sync%n", &string_length);
	write(video_FD, VIDEO_BUF, string_length);
	...
	while(1)
	{
		if (!found_trigger)
		{
			...
			if (last != -1 && abs(last - current) > 3000)
			{
				...
				interval_timer_start.it_interval.tv_nsec = ADC_SWEEP_SCALE * multiplier;
				interval_timer_start.it_value.tv_nsec = ADC_SWEEP_SCALE * multiplier;
				int ret = timer_settime (interval_timer_id, 0, &interval_timer_start, NULL);
				// http://man7.org/linux/man-pages/man2/timer_settime.2.html
				if (ret != 0)
				{
					ret = timer_settime (interval_timer_id, 0, &interval_timer_start, NULL);
				}
		...
		{
			while(read(key_FD, KEY_BUF, KEY_SIZE)!=0);
			if (KEY_BUF[0] == '1')
			{
				if (multiplier > 1)
				{
