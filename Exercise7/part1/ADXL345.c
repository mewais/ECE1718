...
void Pinmux_Config(){
    ...
    *(unsigned int*)(sysmgr_base + SYSMGR_I2C0USEFPGA) = 0;
    *(unsigned int*)(sysmgr_base + SYSMGR_GENERALIO7) = 1;
    *(unsigned int*)(sysmgr_base + SYSMGR_GENERALIO8) = 1;
}
...
void I2C0_Init(){
    ...
    *(unsigned int*)(i2c_base + I2C0_FS_SCL_HCNT) = 60 + 30; // 0.6us + 0.3us
    *(unsigned int*)(i2c_base + I2C0_FS_SCL_LCNT) = 130 + 30; // 1.3us + 0.3us
    
    // Enable the controller
    *(unsigned int*)(i2c_base + I2C0_ENABLE) = 1;
    
    // Wait until controller is enabled
    while(((*(unsigned int*)(i2c_base + I2C0_ENABLE_STATUS)&0x1)) == 0){}
...
void I2C0_Enable_FPGA_Access(){
    ...
    *(unsigned int*)(rstmgr_base + RSTMGR_BRGMODRST) = 0;
    
    // Enable non-secure masters to access I2C0
    *(unsigned int*)(l3regs_base + L3REGS_L4SP) = *(unsigned int*)(l3regs_base + L3REGS_L4SP) | 0x4;
    
    // Enable non-secure masters to access pinmuxing registers (in sysmgr)
    *(unsigned int*)(l3regs_base + L3REGS_L4OSC1) = *(unsigned int*)(l3regs_base + L3REGS_L4OSC1) | 0x10;
}
...
void ADXL345_REG_WRITE(uint8_t address, uint8_t value){
    ...
    *(unsigned int*)(i2c_base + I2C0_DATA_CMD) = address + 0x400;
    
    // Send value
    *(unsigned int*)(i2c_base + I2C0_DATA_CMD) = value;
}
...
void ADXL345_REG_READ(uint8_t address, uint8_t *value){
    ...
    *(unsigned int*)(i2c_base + I2C0_DATA_CMD) = address + 0x400;
    ...
    // Send read signal
    *(unsigned int*)(i2c_base + I2C0_DATA_CMD) = 0x100;
    
    // Read the response (first wait until RX buffer contains data)  
    while (*(unsigned int*)(i2c_base + I2C0_RXFLR) == 0){}
    *value = *(unsigned int*)(i2c_base + I2C0_DATA_CMD);
}
...
void ADXL345_REG_MULTI_READ(uint8_t address, uint8_t values[], uint8_t len){
    ...
    *(unsigned int*)(i2c_base + I2C0_DATA_CMD) = address + 0x400;
    ...
    int nth_byte=0;
    while (len){
        if (*(unsigned int*)(i2c_base + I2C0_RXFLR) > 0){
            values[nth_byte] = *(unsigned int*)(i2c_base + I2C0_DATA_CMD);
            nth_byte++;
            len--;
        }
    }
